...
\section[Starke und schwache Regeln]{Starke und schwache Regeln\protect\footnote{\label{fn-autor-topo}Dies ist eine vorläufige Fassung der einleitenden Abschnitte eines
		...
		Wortstellung vgl.\ vorerst Höhle, Explikationen für "`\isi{normale Betonung}"'
		und "`normale \isi{Wortstellung}"', in: \textit{Satzglieder im Deutschen}.~– Tübingen:
...
\ex\label{ex:1-2-1b} The vast majority of languages have several variant orders but a single \isi{dominant} one.
\ex\label{ex:1-2-1c} Logically, there are six possible orders: \isi{SVO}, \isi{SOV}, \isi{VSO}, \isi{VOS}, \isi{OSV}, and
\isi{OVS}.
\ex\label{ex:1-2-1d} Of these six, however, only three normally occur as \isi{dominant} orders.
...
\end{xlist}
\end{exe}
Eine nähere Erläuterung dafür, wodurch sich eine \isi{dominante Stellung} gegenüber anderen Stellungen auszeichnet, gibt es nicht.
...
In L\textsubscript{i} ist genau 1 \isi{Wortstellung} \isi{dominant}. (Vgl.\ (\ref{ex:1-2-1b}).)
\end{xlist}
\end{exe}
...
the vast majority of languages have a \isi{dominant} order."' \citep[77]{Greenberg1963}
\end{exe}
Wenn die Stellung der Adjektive nur "`sometimes"' variiert, muß sie in den meisten
Sprachen stabil sein; wenn "`the vast majority of languages"' eine \isi{dominante Stellung}
...
L\textsubscript{i} hat ein Element aus WO\textsubscript{t} als \isi{dominante Wortstellung}. (Vgl.\ (\ref{ex:1-2-1d}).)
...
%page 8
aufweist und dennoch zum Normaltypus der Sprachen gehört, wie er in (\ref{ex:1-2-4b}) gekennzeichnet ist; diese Stellungen dürfen in L\textsubscript{l} nur nicht \isi{dominant} sein.
...
\ssubsection{}%2.2.
\label{subsec:1-2.2}
Im abschließenden Abschnitt~5 verwendet Greenberg den Ausdruck "`\isi{dominant}"' unvermittelt in völlig anderer Weise. Er spricht dort von "`dominance of a particular order over its alternative"' und sagt u.\,a.:
\begin{exe}
\ex\label{ex:1-2-5}
...
the pronoun precedes, we will say that VO is \isi{dominant} over \isi{OV} since \isi{OV}
...
\isi{Wortstellung} im Sinne von (\ref{ex:1-2-2b}) bereits etabliert ist. Für nominale Subjekte und Objekte z.\,B.\ ist im Französischen\il{Französisch} wie im Englischen\il{Englisch} S$>$V$>$O die (einzelsprachlich) \isi{dominante Stellung}, völlig unabhängig davon, welche Stellung ein pronominales Objekt einnimmt. In (\ref{ex:1-2-5}) heißt es, V$>$O sei "`\isi{dominant}"' gegenüber O$>$V \textit{deshalb weil} (oder: insofern als) nominale O$>$V"=Stellung nur zusammen mit pronominaler O$>$V"=Stellung
...
im Französischen\il{Französisch}) vorkommt. Das hat nur Sinn, wenn "`\isi{dominant}"' hier in einem typologischen Sinne gemeint ist und nicht, wie in (\ref{ex:1-2-2b}), in Hinsicht auf eine \isi{Einzelsprache}. Dies kann man so erläutern:
\begin{exe}
\ex\label{ex:1-2-6}
Eine \isi{Wortstellung} A$>$B ist (hinsichtlich der Eigenschaft \emph{E}) typologisch \isi{dominant} gegenüber der (\textsq{rezessiven}) \isi{Wortstellung} B$>$A gdw.\ (a) und (b)
gelten:
\begin{xlist}
\ex\label{ex:1-2-6a} 
\begin{xlist}
...
"`\isi{dominant}"' nicht als "`typologisch \isi{dominant}"' im Sinne von (\ref{ex:1-2-6}) interpretiert.
...
dann typologisch \isi{dominant} nennen würde, wenn (\ref{ex:1-2-6bi}) nicht erfüllt wäre, wäre dies
...
etwa ist O$>$V (einzelsprachlich) \isi{dominant} und V$>$O inexistent; trotzdem ist V$>$O
nach (\ref{ex:1-2-5}) typologisch \isi{dominant}. Dementsprechend ist es auch möglich, daß eine typologisch \isi{dominante Wortstellung} A$>$B in weniger Sprachen vorkommt als ihr \textsq{rezessives} Gegenstück.
...
more frequent than the \isi{dominant}. Dominance [\ldots] relations can be derived
...
\isi{Verb}) gegenüber O$>$V typologisch \isi{dominant} ist (weil die Kombination pronominal
V$>$O/""\isi{nominal} O$>$V in keiner Sprache vorkommt).\footnote{%
  ...
  (hinsichtlich der Stellung von nominalen Objekten zum \isi{Verb}) typologisch \isi{dominant} ist.%
}
...
the notion of \textsc{\isi{dominant} order} of syntactic elements and explains (p.\,76) that the ``dominance is not based on its more frequent occurrence'' (a \isi{dominant} order is not that alternative which is more frequent than its opposite,
the \textsqe{recessive} order) but on the fact that the \isi{dominant} order can always occur while its opposite is present only under specified conditions, i.e., in cooccurrence with another, \textsqe{harmonic}, construction. These conditions are
...
\textsqe{recessive alternatives} to a \textsqe{\isi{dominant} order} are
...
"`(einzelsprachlich) \isi{dominant}"' wird bei Greenberg überhaupt nicht
erläutert; "`typologisch \isi{dominant}"' wird zwar erläutert, hat mit
(einzelsprachlich) \isi{dominant} oder mit "`normal"' in Dane\v{s}s Sinn aber
nur indirekt ewas zu tun.
%page 11

\ssubsection{}%2.3.
\label{subsec:1-2.3}
Aber was meint Greenberg mit "`(einzelsprachlich) \isi{dominant}"'? Möglicherweise tatsächlich so etwas wie Jakobsons \textsq{stilistisch neutrale} und Dane\v{s}s \textsq{normale}
(usual) \isi{Wortstellung}. Als Indiz kann man einige andere Adjektive auf"|führen, die
Greenberg, wie es scheint, in demselben Sinn wie "`\isi{dominant}"' gebraucht.

An mehreren Stellen steht "`normal"', wo man aufgrund des Textzusammenhangs
"`\isi{dominant}"' erwarten könnte. So werden die Sprachen gemäß (\ref{ex:1-2-1}) u.\,a.\ danach klassifiziert, ob ihre dominante Abfolge von \isi{Subjekt}, Objekt und Vollverb V$>$S$>$O, S$>$V$>$O
...
word order is verb"=subject"=object"'. Hier wird also "`normal"' in genau derselben Weise gebraucht wie zuvor "`\isi{dominant}"'. Ebenso \zb S.\,79 (Universal~4), S.\,83f (mehrfach,
...
Es scheint danach, daß durch "`\isi{dominant}"' eine Eigenschaft bezeichnet
wird, die eng verwandt ist mit intuitiven Begriffen der Normalität und
des Üblichen. Wenn man demzufolge annimmt, daß "`\isi{dominant}"' im
...
\begin{exe}
\ex\label{ex:1-2-11}
"`Languages with \isi{dominant} \isi{VSO} order are always prepositional."' \citep[78]{Greenberg1963}
\end{exe}
Diese Parallelität scheint dafür zu sprechen, daß "`basic"' in (\ref{ex:1-2-10}) synonym mit "`\isi{dominant}"' in (\ref{ex:1-2-11}) ist. Dem steht jedoch der Wortgebrauch S.\,79f entgegen:
\begin{exe}
\ex\label{ex:1-2-12}
...
jede \textsq{basic order} zugleich eine \textsq{\isi{dominant} order}. Aber was ist dann eine \textsq{basic order}? Die einzige konsistente Interpretation, die mir möglich scheint, ergibt sich aus
...
die Folge S$>$V$>$O als einzige \isi{dominante Stellung}.
\end{exe}
...
\end{xlist}
\end{exe}
Diese Sätze sind intuitiv in keiner Weise \textsq{markiert}, sie sind stilistisch neutral in Jakobsons Sinn. Ich sehe keinen Grund, anzunehmen, ihre \isi{Wortstellung} sei nicht \isi{dominant}. Sie weisen jedoch nicht S$>$V$>$O"=Stellung auf, sondern S$>$O$>$V.
...
weisen daher S$>$O$>$V"=Stellung auf, oder genauer:\footnote{%
  ...
  ist.~-- Vielleicht findet sich eine Interpretation von "`\isi{dominant}"',
...
unsere Beobachtungen über das Vorkommen von mehreren Hilfsverben in (\ref{ex:1-3-10})/""(\ref{ex:1-3-11}) berücksichtigen, können wir die \isi{dominante Wortstellung} wie in (\ref{ex:1-3-15}) kennzeichnen:
\begin{exe}
\ex\label{ex:1-3-15}
...
Greenbergs Behauptung, das Deutsche\il{Deutsch} habe S$>$V$>$O als \isi{dominante Wortstellung}.
...
\end{xlist}
\end{exe}
Nichts spricht dafür, dies nicht für eine \isi{dominante Wortstellung} zu halten; die Sätze
...
der
%page 22
ersten Position hinsichtlich seiner syntaktischen Funktion im Satz frei ist: Es kann~-- mindestens~-- ein \isi{adverbialer Ausdruck} oder ein \isi{Subjekt} sein. Wir kommen also zu
dem Schluß, daß die \isi{dominante Wortstellung} durch ein \isi{Schema} wie (\ref{ex:1-3-23}) zu kennzeichnen ist; dabei symbolisiert "`\textit{K}"' die funktional unbestimmte erste Position:
\begin{exe}
\ex\label{ex:1-3-23}
...
S$>$V$>$O als \isi{dominante Wortstellung}.
